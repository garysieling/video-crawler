...
object ColorList {
  def main(args: Array[String]): Unit = {
    val test: ColorListSpark = new ColorListSpark
    test.entryPoint(args)
  }
}

class ColorListSpark {
  private val log: Logger = LoggerFactory.getLogger(classOf[ColorListSpark])
  ...
  // policy
  @Parameter(names = Array("-accessKey"), description = "your aws accesskey") private val accessKey: String = "AKIAJCQHPATID6CLJLKA"
  @Parameter(names = Array("-secretKey"), description = "your aws secretKey") private val secretKey: String = "HOG1F4UYrJ4rQjWXAs2IT4S0VX9946yhlLy7gRPv"
  @Parameter(names = Array("-region"), description = "your aws region") private val region: Regions = Regions.US_EAST_1
  @Parameter(names = Array("-debug"), description = "enable spark debug mode") private val debug: Boolean = true
  @Parameter(names = Array("-execute"), description = "run spark step after cluster creation") private val execute: Boolean = true
  @Parameter(names = Array("-upload"), description = "upload uber jar") private val upload: Boolean = true
  // default emr version runs spark2, so make sure to update spark version in
  // parent pom (default is spark1)
  @Parameter(names = Array("-emrVersion"), description = "version of aws emr software stack") private val emrVersion: String = "emr-5.4.0"
  @Parameter(names = Array("-bucketName"), description = "aws s3 bucket name for logs and uber jar") private val bucketName: String = "sparkuser"
  @Parameter(names = Array("-keepAlive"), description = "keep cluster alive after execution of spark step") private val keepAlive: Boolean = false
  @Parameter(names = Array("-instanceType"), description = "emr master and slave instance type") private val instanceType: String = "m3.xlarge"
  @Parameter(names = Array("-instanceCount"), description = "instance count, 1 will be master and rest slaves") private val instanceCount: Int = 1
  @Parameter(names = Array("-uberJar"), description = "uber jar with spark step to execute") private val uberJar: String =
    "D:\\projects\\clones\\dl4j-examples\\dl4j-spark-examples\\dl4j-spark\\target\\dl4j-spark-0.9.1-bin.jar"
  @Parameter(names = Array("-className"), description = "class name of spark step to execute") private val className: String =
    "ColorList"

  def entryPoint(args: Array[String]) {
    val jcmdr = new JCommander()
    try {
      jcmdr.parse(args: _*)
    }
    catch {
    ...
    val emr: AmazonElasticMapReduce = builder.build
    val steps = new java.util.ArrayList[StepConfig]

    if (upload) {
      log.info("uploading uber jar " + uberJar)
      ...
      val s3Client: AmazonS3 = s3builder.build

      if (!s3Client.doesBucketExist(bucketName)) {
        s3Client.createBucket(bucketName)
      }

      val uberJarFile: File = new File(uberJar)
      s3Client.putObject(new PutObjectRequest(bucketName, uberJarFile.getName, uberJarFile))
    }

    if (debug) {
      log.info("enable debug")
      val stepFactory: StepFactory = new StepFactory(builder.getRegion + ".elasticmapreduce")
    ...
    if (execute) {
      ...
      sparkStepConf.withArgs(
        "spark-submit", "--deploy-mode", "cluster", "--class", className, getS3UberJarUrl, "-useSparkLocal", "false")
      var action: ActionOnFailure = ActionOnFailure.TERMINATE_JOB_FLOW

      if (keepAlive) {
        action = ActionOnFailure.CONTINUE
      }

      val sparkStep: StepConfig =
        new StepConfig()
          .withName("Spark Step")
  ...
  def getS3UberJarUrl: String = {
    return getS3BucketUrl + "/" + new File(uberJar).getName
  }

  def getS3BucketUrl: String = {
    return "s3://" + bucketName
  }

  def getS3BucketLogsUrl: String = {
    return getS3BucketUrl + "/logs"
  }

  def getCredentialsProvider: AWSStaticCredentialsProvider = {
...
class ColorListProcess {
  private val log: Logger = LoggerFactory.getLogger(classOf[ColorListProcess])
  ...
  def main(args: Array[String]) {
    new ColorListProcess().entryPoint(args)
  }

  private def entryPoint(args: Array[String]) {
    log.info("Called map code")
  }
}